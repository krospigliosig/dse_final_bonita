...
The project **rabbitmq-connector** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/rabbitmq-connector.git
cd rabbitmq-connector/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `rabbitmq-connector-1.0-SNAPSHOT.jar`
